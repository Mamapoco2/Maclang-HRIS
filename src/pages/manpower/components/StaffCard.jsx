...
const StaffCard = ({ name, role, employmentType, aligned, image }) => {
  ...
  return (
    ...
    >
      ...
      <div className="text-left leading-tight max-w-[150px] wrap-break-words">
        <div
          className={`font-semibold text-sm ${isVacant ? "text-gray-400" : ""}`}
        >
          {name}
        </div>
        <div
          className={`text-xs ${isVacant ? "text-gray-400" : "text-gray-500"}`}
        >
          {role}
        </div>
      </div>
