...
export const RatingBadge = ({ rating }) => {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-semibold border ${cfg.bg} ${cfg.text} ${cfg.border}`}>
      {rating}
    </span>
  );
};
...
export const TrendIndicator = ({ trend, amount }) => (
  <div className={`flex items-center gap-0.5 text-xs font-semibold ${trend === "up" ? "text-emerald-600" : "text-red-500"}`}>
...
export const ScoreTags = ({ spms, appraisal }) => (
  <div className="flex items-center gap-1.5 mt-1">
    <span className="text-xs text-gray-400">
      SPMS <span className="font-semibold text-gray-600 tabular-nums">{spms}</span>
    </span>
    <span className="text-gray-300">·</span>
    <span className="text-xs text-gray-400">
      AR <span className="font-semibold text-gray-600 tabular-nums">{appraisal}</span>
    </span>
  </div>
);
