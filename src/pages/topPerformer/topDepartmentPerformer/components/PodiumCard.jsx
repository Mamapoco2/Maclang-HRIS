...
export const PodiumCard = ({ player }) => {
  ...
  return (
    <motion.div
      ...
      transition={{
        delay: (player.rank - 2) * 0.08,
        duration: 0.35,
        ease: "easeOut",
      }}
      whileHover={{ y: -2, transition: { duration: 0.2 } }}
      className="bg-white border rounded-xl overflow-hidden cursor-default"
    ...
    >
      ...
      <div className="p-5">
        <div className="flex items-start gap-3">
          {/* Rank */}
          <div
            className={`flex-shrink-0 w-7 h-7 rounded-full border flex items-center justify-center text-sm font-bold ${style.rankBg}`}
          >
            {player.rank}
          </div>
          ...
          {/* Avatar */}
          <div
            className={`flex-shrink-0 w-11 h-11 rounded-full flex items-center justify-center text-sm font-semibold ${color}`}
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="text-right flex-shrink-0">
                <div className={`text-2xl font-bold ${style.scoreFg}`}>
                  {player.score}
                </div>
                <div className="text-xs text-gray-400">/ 100</div>
                <TrendIndicator
                  trend={player.trend}
                  amount={player.trendAmount}
                />
              </div>
            </div>
            <div className="mt-3">
