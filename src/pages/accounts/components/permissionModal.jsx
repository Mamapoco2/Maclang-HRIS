...
export default function PermissionsModal({ user, open, onClose, onSave, saving = false }) {
  ...
  useEffect(() => {
    ...
    PERMISSION_GROUPS.forEach(({ permissions }) =>
      permissions.forEach(({ key }) => { initial[key] = user.permissions?.includes(key) ?? false; }),
    );
    setSelected(initial);
  ...
  const toggleGroup = (permissions) => {
    ...
    setSelected((prev) => { const next = { ...prev }; keys.forEach((k) => (next[k] = !allChecked)); return next; });
  };
            ...
            return (
              <div key={group} className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                ...
                <div className="flex items-center gap-2.5 px-4 py-2.5 bg-gray-50 border-b border-gray-100">
                  ...
                  />
                  <label htmlFor={`group-${group}`} className="text-xs font-semibold text-gray-700 cursor-pointer select-none flex-1 uppercase tracking-wide">
                    {group}
                  </label>
                  {someChecked && <span className="text-[10px] text-gray-400">partial</span>}
                </div>

                {/* Permission items */}
                <div className="grid grid-cols-2 divide-x divide-gray-50">
                  {permissions.map(({ key, label }) => (
                    <div key={key} className="flex items-center gap-2.5 px-4 py-2 border-b border-gray-50 last:border-b-0">
                      <Checkbox
                        id={key} checked={!!selected[key]}
                        onCheckedChange={() => toggle(key)}
                        className="border-gray-300"
                      />
                      <label htmlFor={key} className="text-xs text-gray-600 cursor-pointer select-none">
                        {label}
                      </label>
                    </div>
                  ))}
